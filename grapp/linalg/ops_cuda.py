"""
Linear operators compatible with scipy, using CuPy when available.
"""
...
try:
    import cupy as xp
    _device_ctx = xp.cuda.Device
    _CUPY_AVAILABLE = True
except ImportError:
    import numpy as xp  # type: ignore
    import contextlib
    _device_ctx = lambda _: contextlib.nullcontext()
...
class SciPyXOperator(LinearOperator):
    ...
    def __init__(
        ...
        if _CUPY_AVAILABLE and self._device is None:
            raise ValueError("grg.device must be set when using CuPy")
        assert (
    ...
    def _matmat_helper(
        self, other_matrix: numpy.typing.NDArray, mult_dir: TraversalDirection, out=None
    ):
        kwargs = {}
        ...
        if use_M:
            if mult_dir == _DOWN:
                ...
                with _device_ctx(self._device):
                    M = xp.zeros((A.shape[0], self.grg.num_mutations))
            kwargs["miss"] = M
        ...
        if out is not None:
            out[:] = result   # D2D copy via cudaMemcpyPeer when devices differ
            return out
        return result

    def _matmat(self, other_matrix, out=None):
        return self._matmat_helper(other_matrix, _flip_dir(self.direction), out=out)

    def _rmatmat(self, other_matrix, out=None):
        return self._matmat_helper(other_matrix, self.direction, out=out)

    def _matvec(self, vect):
        if vect.ndim != 2:
            with _device_ctx(self._device):
                vect = xp.asarray([vect]).T  # Column vector (Mx1)
        return self._matmat(vect)

    def _rmatvec(self, vect):
        if vect.ndim != 2:
            with _device_ctx(self._device):
                vect = xp.asarray([vect]).T  # Column vector (Nx1)
...
class _SciPyStandardizedOperator(LinearOperator):
    ...
    def __init__(
        ...
        with _device_ctx(self._device):
            ...
            with xp.errstate(invalid="raise"):
                self.inverse_sigma = xp.zeros(variance.shape)
                xp.sqrt(
                    xp.power(
...
class SciPyStdXOperator(_SciPyStandardizedOperator):
    ...
    def _matmat_direction(self, other_matrix, direction, out=None):
        mult_dir = _flip_dir(direction)
        with xp.errstate(divide="raise"):
            if direction == _UP:
                vS = (
                ...
                consts = xp.array(
                    [xp.sum(self.mult_const * self.freqs * vS, axis=1)]
                ).T
    ...
    def _matmat(self, other_matrix, out=None):
        return self._matmat_direction(other_matrix, self.direction, out=out)

    def _rmatmat(self, other_matrix, out=None):
        return self._matmat_direction(other_matrix, _flip_dir(self.direction), out=out)
...
class MultiSciPyXOperator(LinearOperator):
    ...
    def __init__(
        ...
        self._output_device = self.operators[0]._device if self.operators else None
    ...
    def _matmat_helper(self, other_matrix, direction, op_method):
        ...
        if direction == pygrgl.TraversalDirection.UP:
            # Pre-alloc one output buffer per op on _output_device; D2D copies in threads (parallel)
            n_rows = self.operators[0].shape[0]
            parts = []
            for _ in self.operators:
                with _device_ctx(self._output_device):
            ...
            for op, part in zip(self.operators, parts):
                ...
                if _CUPY_AVAILABLE:
                    with _device_ctx(op._device):
                        sub_matrix = xp.asarray(sub_matrix)
                futures.append(self.scheduler.submit(op.grg, op_method, op, sub_matrix, part))
                start = end
            for f in futures:
                f.result()
            if _CUPY_AVAILABLE:
                ...
                return sum(parts[1:], parts[0])
        # For DOWN, pass non-overlapping slices of pre-alloc'd output; D2D copies in threads (parallel)
        else:
            total_rows = sum(op.shape[0] for op in self.operators)
            with _device_ctx(self._output_device):
            ...
            for op in self.operators:
                ...
                op_input = other_matrix
                if _CUPY_AVAILABLE:
                    with _device_ctx(op._device):
                        op_input = xp.asarray(other_matrix)
                futures.append(self.scheduler.submit(op.grg, op_method, op, op_input, out_slice))
                start = end
            for f in futures:
                f.result()
            return output
