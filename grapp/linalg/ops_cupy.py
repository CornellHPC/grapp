"""
CuPy-native linear operators for single-GRG matrix multiplications.
...
Stream model
------------
All intermediate arrays are allocated and all kernels are launched on
self._device's null (default) stream.  The null stream is device-global
and host-thread-agnostic under CuPy's default configuration, so work
submitted from different Python threads to the same device is implicitly
serialized.

When an `out` buffer on a different device is provided to _matmat/_rmatmat,
_xdev_copy(out, result) is called:
  1. self._device's current stream is synchronized, guaranteeing that result
     is fully written before the D2D copy begins.
  2. The D2D copy kernel is queued on out.device's null stream.
     Subsequent operations on out.device will be serialized after the copy.

NOTE: if CUDA_API_PER_THREAD_DEFAULT_STREAM is ever enabled, the null-stream
serialization guarantee breaks.  Multi operators that sum pre-allocated output
buffers would then need an explicit device sync after collecting futures.
"""
...
def _xdev_copy(dst, src) -> None:
    ...
    if force_host_xdev_copy:
        ...
        if hasattr(src, "device") and hasattr(dst, "device") and src.device != dst.device:
            # Sync the source stream before the cross-device copy.
            # No implicit ordering exists between separate device streams.
...
def _xdev_asarray(src):
    ...
    xp.asarray is a no-op when src is already a CuPy array — even if it lives
    on a different device — so an explicit D2D copy is required when devices
    differ.
    """
    if force_host_xdev_copy and hasattr(src, "get"):
        return xp.asarray(src.get())
    dev = getattr(src, "device", None)
    # Host arrays expose .device == 'cpu' (a str) under NumPy >= 2.0; only treat
    # genuine CuPy devices (which carry an .id) as candidates for a D2D copy.
    if dev is not None and hasattr(dev, "id") and dev.id != xp.cuda.Device().id:
        # CuPy cannot copy a non-contiguous array across devices; make the source
...
class CuPyXOperator(LinearOperator):
    ...
    def _rmatmat(self, other_matrix, out=None):
        _call_counts[f"{type(self).__name__}._rmatmat"] = _call_counts.get(f"{type(self).__name__}._rmatmat", 0) + 1
